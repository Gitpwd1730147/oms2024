...
import com.tzb.oms.exception.BusinessException;
...
public class TokenUtils {
    ...
    private static final String CLAIM_NAME_USERPWD = "CLAIM_NAME_USERPWD";
    ...
    private String sign(CurrentUser currentUser,String securityKey){
        String token = JWT.create()
                ...
                .withClaim(CLAIM_NAME_USERPWD,currentUser.getUserPwd())
    ...
    public CurrentUser getCurrentUser(String token) {
        ...
        String userPwd = decodedJWT.getClaim(CLAIM_NAME_USERPWD).asString();//用户密码

        if(StringUtils.isEmpty(userCode) || StringUtils.isEmpty(userName)){
            throw new BusinessException("令牌缺失用户信息，请登录！");
        }
        return new CurrentUser(userId, userCode, userName,userPwd);
    }
